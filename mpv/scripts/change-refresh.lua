--[[
    ...
    Modified to remove default keybindings
    License: MIT
    ...
    They are also completely independent from the usual automatic reversion system, so you'll have to handle that yourself.
...
local options = {
    ...
    --only used if detect_display_resolution is false
    original_width = 1920,
...
function setDefault()
    ...
    msg.info('set ' .. var.original_width .. "x" .. var.original_height .. " " .. var.original_fps .. "Hz as default display rate")
    osdMessage('Change-Refresh: set ' .. var.original_width .. "x" .. var.original_height .. " " .. var.original_fps .. "Hz as defa/ult display rate")
end
...
function scriptMessage(width, height, rate, display)
    ...
    msg.verbose('received script message: ' .. width .. ' ' .. height .. ' ' .. rate .. ' ' .. display)
...
mp.add_key_binding(nil, "match-refresh", matchVideo)

--reverts monitor to original refreshrate
mp.add_key_binding(nil, "revert-refresh", revertRefresh)

--switches between using estimated and specified fps property
mp.add_key_binding(nil, 'toggle-fps-type', toggleFpsType)

--set the current resolution and refresh rate as the default
mp.add_key_binding(nil, "set-default-refresh", setDefault)
